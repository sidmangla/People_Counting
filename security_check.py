...
class SecurityCheck():
	...
	def load_file(self,file):
		with open(file) as f:
			...
			self.entry_point = int(info["entry_point"])
			self.exit_point = int(info["exit_point"])
			...
			self.security_check = int(info["security_check"])
			self.sec_trigger = info["security_trigger"]
			self.sec_confidence = int(info["security_confidence"])
			self.sec_trig_xlimit = int(info["sec_trig_xlimit"])
			self.sec_trig_ylimit = int(info["sec_trig_ylimit"])
...
if __name__ == '__main__':
	try:
		...
		input_path,fps,output = project.load_file(file)
		...
		project.x_res = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
		project.y_res  = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) 
		out=cv2.VideoWriter(output, fourcc,fps , (project.x_res,project.y_res), True)
		while cap.isOpened(): 
			ret,img=cap.read()
			if ret is True:
			...
			img = project.Security_entryGate(img,result)
			out.write(img)
		cap.release()
		out.release()
		f.close()
